...
from permissions import load_superuser_emails, load_view_permissions_cache
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # AD/Entra permission source not yet wired — empty stub cache (see
        # permissions.py). No DB connection needed at startup; each request opens
        # its own pooled connection on demand.
    ...
    logger.info(
        "Startup complete — DB: %s/%s (auth=%s), MAX_ROWS: %d, superusers: %d",
