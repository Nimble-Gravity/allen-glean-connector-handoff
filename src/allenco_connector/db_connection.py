...
# SQL Server DATETIMEOFFSET is returned over ODBC as SQL_SS_TIMESTAMPOFFSET (-155),
# a 20-byte struct pyodbc has no built-in decoder for — a fetch of such a column
# raises "ODBC SQL type -155 is not supported". EMS travel views (e.g. rpt.v_TravelAir
# / rpt.v_TravelGround) use it, so register an output converter on every connection.
SQL_SS_TIMESTAMPOFFSET = -155


def handle_datetimeoffset(raw: bytes) -> datetime:
    """Decode a SQL Server DATETIMEOFFSET (SQL_SS_TIMESTAMPOFFSET) value to a datetime.

    The 20-byte layout is year, month, day, hour, minute, second, nanoseconds, then
    the timezone hour/minute offset. Nanoseconds are truncated to microseconds.
    """
